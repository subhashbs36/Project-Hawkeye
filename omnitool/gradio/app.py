...
INTRO_TEXT = "Hawkeye support **OpenAI (4o/o1/o3-mini), DeepSeek (R1), Qwen (2.5VL), Anthropic Computer Use (Sonnet), Ollama (R1) or LLM Studio(Qwen2.5).**"
...
def validate_auth(provider: APIProvider, api_key: str | None):
    # Skip validation for local providers (Ollama and LM Studio)
...
def valid_params(user_input, state):
    ...
    # Only check API key if provider is not Ollama
...
def process_input(message: str, state: dict):
    if not message:
    ...
    if state["provider"] not in ["ollama", "lmstudio"]:
        if not state["api_key"]:
            errors.append("LLM API Key is not set")
        validation_error = validate_auth(state["provider"], state["api_key"])
        if validation_error:
    ...
    state['chatbot_messages'].append((message, None))
...
with gr.Blocks(theme=gr.themes.Default(), fill_height=True, fill_width=True) as demo:
    ...
    gr.HTML('<h1 style="text-align: center; font-weight: normal; font-size: 60px;">Hawk<span style="font-weight: bold;">Eye</span></h1>', padding=False)
    # else:
    #     gr.Markdown("# HawkEye")
    ...
    with gr.Row():
        with gr.Column(scale=2):
            chatbot = gr.Chatbot(label="Chatbot History", autoscroll=True, height=530)

        with gr.Column(scale=3):
            iframe = gr.HTML(
                f'<iframe src="http://{args.windows_host_url}/vnc.html?view_only=1&autoconnect=1&resize=scale" width="100%" height="530" allow="fullscreen"></iframe>',
                container=False,
                elem_classes="no-padding"
            )
    msg = "Type a message and press submit to start Hawkeye. Press stop to pause, and press the trash icon in the chat to clear the message history."
    gr.Markdown(msg, elem_classes="markdown-text")

    with gr.Row():
        with gr.Column(scale=8):
            chat_input = gr.Textbox(show_label=False, placeholder="Type a message to send to Hawkeye ...", container=False)
    ...
    with gr.Accordion("Settings", open=True): 
        with gr.Row():
            with gr.Column():
                model = gr.Dropdown(
                    label="Model",
                    choices=["omniparser + gpt-4o", "omniparser + o1", "omniparser + o3-mini", 
                            "omniparser + R1", "omniparser + qwen2.5vl", 
                            "claude-3-5-sonnet-20241022", "omniparser + ollama",
                            "omniparser + lmstudio"],
    ...
    gr.HTML("""
        ...
        .no-padding {
            padding: 0 !important;
            margin: 0 !important; /* Set no-padding margin to 0 */
        }
        .no-padding > div {
            padding: 0 !important;
            margin: 0 !important; /* Set no-padding margin to 0 */
        ...
        .markdown-text p {
            ...
            margin: 0; /* Set paragraph margin to 0 */
        }
        h1 {
            color: #4A90E2; /* Primary color for headings */
            text-align: center;
            font-weight: bold;
            margin: 0; /* Set heading margin to 0 */
            padding: 0; /* Set heading padding to 0 */
    ...
    def update_model(model_selection, state):
        ...
        elif model_selection == "omniparser + lmstudio":
        ...
        # Don't show API key placeholder for Ollama or LM Studio
        api_key_placeholder = "" if default_provider_value in ["ollama", "lmstudio"] else f"{default_provider_value.title()} API Key"

        # Update state
        state["provider"] = default_provider_value
        # Don't set API key for Ollama or LM Studio
        state["api_key"] = "" if default_provider_value in ["ollama", "lmstudio"] else state.get(f"{default_provider_value}_api_key", "")
        ...
        api_key_update = gr.update(
            ...
            value=state["api_key"],
            interactive=default_provider_value not in ["ollama", "lmstudio"]  # Disable API key input for Ollama and LM Studio
        )
